      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        @for (store of stores(); track store.id) {
          <div class="rounded-apple-xl overflow-hidden bg-white dark:bg-carbon-900 border border-black/[0.06] dark:border-carbon-border shadow-apple-card dark:shadow-apple-card-dark transition-all duration-300 hover:border-fluor/50 hover:shadow-2xl flex flex-col">
            ...
            <div class="h-36 bg-gradient-to-r from-zinc-800 to-carbon-950 relative p-6 flex items-end">
              <div class="absolute inset-0 opacity-20 bg-[radial-gradient(#D4FF00_1px,transparent_1px)] [background-size:16px_16px]"></div>
              
              <!-- Avatar / Logo Tienda -->
              <div class="w-16 h-16 rounded-apple-md bg-white dark:bg-carbon-850 p-2.5 shadow-xl border-2 border-white dark:border-carbon-border flex items-center justify-center translate-y-6">
            ...
            <div class="pt-10 p-6 flex-1 flex flex-col justify-between space-y-4">
              <div>
                <div class="flex items-center gap-2">
                  <h2 class="text-2xl font-display font-bold text-zinc-950 dark:text-white tracking-tight">
                    {{ store.name }}
                  </h2>
                ...
                <div class="mt-4">
                  ...
                  <div class="flex flex-wrap gap-1.5">
                    @for (brand of store.brands; track brand) {
              ...
              <!-- Footer de la Card con WhatsApp Directo -->
              <div class="pt-4 border-t border-zinc-100 dark:border-zinc-800/80 flex items-center justify-between">
                <span class="text-xs font-display font-bold text-fluor">
                  {{ store.productsCount }} artículos disponibles
                </span>
...
export class StoresComponent {
  stores = signal<OfficialStore[]>([
    {
      id: 1,
      name: 'HobbyMotors',
      slug: 'hobbymotors',
      description: 'Referente histórico del aeromodelismo argentino con más de 30 años en radiocontrol, motores de competición y repuestos oficiales.',
      brands: ['Futaba', 'O.S. Engines', 'Spektrum', 'Align', 'E-flite', 'Dubro'],
      address: 'Av. del Libertador 4520',
    ...
    },
    {
      ...
      name: 'AeroJols',
      slug: 'aerojols',
      description: 'Comercio especializado en aeromodelos acrobáticos gigantes, transmisiones FrSky y motores nafteros de gran cilindrada.',
      brands: ['FrSky', 'RadioMaster', 'DLE Engines', 'Saito', 'T-Motor', 'Kavan'],
      address: 'Calle 12 N° 840',
